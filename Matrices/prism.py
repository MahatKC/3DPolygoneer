...
def vertices(sides, radius, center):
    ...
    vertices = np.concatenate((np.concatenate((vertices,np.zeros((1,sides))),axis=0)*radius + center,np.ones((1,sides))),axis=0)
    return vertices
...
def create_prism(x,y,z,h,r_bottom,r_top,sides):
    ...
    return np.concatenate((vertices(sides, r_top, np.array([[x],[y],[z+h]])),vertices(sides, r_bottom, np.array([[x],[y],[z]])), np.array([[x],[y],[z+(h/2)],[1]])),axis=1)
